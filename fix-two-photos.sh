...
const heroBg = d.hero_bg_photo || '';
const splitPhoto = d.split_photo || d.office_photo || '';
---
<Layout title="About — TNC.18">
  <Navbar />

  <!-- HERO — hero_bg_photo fills the background -->
  <section class="hero">
    {heroBg && <img src={heroBg} alt="TNC.18" class="hero-bg" loading="eager" />}
  ...
  <!-- SPLIT — split_photo appears on the left -->
  <section class="split">
    <div class="split-inner">
      <div class="img-col">
        {splitPhoto
          ? <img src={splitPhoto} alt="TNC.18 Office" class="office-img" loading="lazy" />
...
<style>
  ...
  .hero-overlay {
    position: absolute; inset: 0;
    background: linear-gradient(to top, rgba(28,26,23,0.88) 0%, rgba(28,26,23,0.3) 100%);
    z-index: 1;
  }
  ...
  .hero h1 {
    ...
    font-weight: 300; color: var(--stone); line-height: 1.1;
  }
  ...
  .team-copy {
    font-family: var(--font-serif); font-size: clamp(1.25rem,2vw,1.6rem);
    font-weight: 300; font-style: italic; color: var(--stone);
    line-height: 1.6; margin: 1.5rem 0 2.5rem;
  }
  .cta {
    font-size: 0.7rem; letter-spacing: 0.16em; text-transform: uppercase;
    color: var(--gold); text-decoration: none;
    border-bottom: 1px solid rgba(166,124,78,0.4); padding-bottom: 2px;
  }
